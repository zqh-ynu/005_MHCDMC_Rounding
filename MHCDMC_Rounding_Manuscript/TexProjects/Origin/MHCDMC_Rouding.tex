...
\begin{figure}[!t]
	...
	\begin{algorithm}[H]
		\caption{Determining Superior and Inferior Servers}
		\begin{algorithmic}[1]\label{alg:DSIS}
			...
			\STATE $a_m \leftarrow \mathop {\arg \max }\nolimits_{{a_i} \in {G_l}} {r_i}$.
			\STATE Rerout the flow from each $a_i\in G_l\backslash\left\{a_m\right\}$ to $a_m$.
			\STATE $\overline{y}_m\leftarrow 1$; $\overline{y}_i \leftarrow 0, \forall a_i \in G_l\backslash\left\{a_m\right\}$.
			\ENDFOR
			\ENDFOR
			\STATE Rerout the flow from $\mathcal{T}$ to server has maximum radii in $\mathcal{I}_j$.
			\ENDIF
			\ENDFOR
...
\begin{figure}[!t]
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]\label{alg:DSIS}
			...
			\ENSURE A fraction solution $\widehat \sigma = ({\widehat x},{\widehat y})$ containing only superior and inferior servers.
			\STATE Initialize $\widehat x \leftarrow \overline x$, $\widehat y \leftarrow \overline y$.
			...
			\STATE $\mathcal{O} \leftarrow \emptyset, \mathcal{I}' \leftarrow \mathcal{I}$; $C_i \leftarrow \emptyset, \forall a_i \in \mathcal{S}$
			\WHILE{$\mathcal{I}' \ne \emptyset$}
			\FORALL{$a_i \in \mathcal{S}$}
			\FORALL{$a_t \in \mathcal{I}'$}
			\IF{$a_t$ intersects $a_i$ and $AC_i \ge \sum\nolimits_{{u_j} \in U} {{{\widehat x}_{tj}}} $}
			\STATE Rerout the flow from $a_t$ to $a_i$, and $C_i \leftarrow C_i \cup \left\{a_t\right\}$.
			\ENDIF
			\ENDFOR
			\ENDFOR
			\IF{$\mathcal{I}' \ne \emptyset$}
			...
			\STATE $a_t \leftarrow \arg\max_{a_i \in \mathcal{I}'}{k_i}$, $\mathcal{O} \leftarrow  \mathcal{O} \cup \left\{a_t\right\}$.
